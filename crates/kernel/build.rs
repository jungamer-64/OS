...
// Note: We intentionally do not parse the target spec JSON in the build script.
// Keeping this build script minimal avoids pulling heavy crates (serde/serde_json)
// into the build dependency graph, keeping iterative dev build times low.
...
fn main() {
    ...
    // Check the build target environment (lightweight)
    validate_target_env();
...
fn validate_target_env() {
    ...
    // Minimal validation to avoid pulling in heavy deps in the build script
    if !target.contains("x86_64") {
        println!(
            "cargo:warning=Building for target '{}'. Ensure this is intended for an x86_64 kernel.",
            target
        );
    }
}
