...
fn kernel_main(boot_info: &'static mut BootInfo) -> ! {
    ...
    match tiny_os::kernel::process::create_user_process() {
        Ok(pid) => {
            ...
            // DEBUG: Verify kernel page table contents
            // User page table verification moved to create_user_page_table() for efficiency
            {
                use x86_64::structures::paging::PageTable;
                use x86_64::registers::control::Cr3;
                let phys_mem_offset = tiny_os::kernel::mm::PHYS_MEM_OFFSET.load(core::sync::atomic::Ordering::Relaxed);
                
                // Show kernel page table
                ...
                for i in 0..512 {
                    if !kernel_pt[i].is_unused() {
                        debug_println!("  Kernel entry {}: addr={:#x}, flags={:?}", 
